...
*/
    ...
    })(); CKEDITOR.tools.createImageData = function (a) { return "data:image/svg+xml;charset\x3dutf-8," + encodeURIComponent('\x3csvg xmlns\x3d"http://www.w3.org/2000/svg" width\x3d"' + a.width + '" height\x3d"' + a.height + '"\x3e\x3c/svg\x3e') };
    CKEDITOR.plugins.add("video", {
        requires: "dialog,fakeobjects", lang: "en,fr", icons: "video", hidpi: !0, onLoad: function () { var a = CKEDITOR.getUrl(this.path + "images/placeholder.png"); CKEDITOR.addCss("img.cke-video{background:#f8f8f8 url(" + a + ") center center no-repeat;outline:1px solid #ccc;outline-offset:-1px;min-width:192px;min-height:108px;max-width:100%;width:auto!important;height:auto!important;}") }, init: function (a) {
            a.addCommand("video", new CKEDITOR.dialogCommand("video", { allowedContent: "video[autoplay,controls,height,loop,muted,preload,!src,width]" }));
            a.ui.addButton("Video", { label: a.lang.video.button, command: "video" }); CKEDITOR.dialog.add("video", this.path + "dialogs/video.js"); a.on("doubleclick", function (a) { var b = a.data.element; b && b.is("img") && !b.isReadOnly() && "video" == b.data("cke-real-element-type") && (a.data.dialog = "video") }); a.addMenuItems && (a.addMenuGroup("video", 11), a.addMenuItems({ video: { label: a.lang.video.title, command: "video", group: "video" } })); a.contextMenu && a.contextMenu.addListener(function (a) {
                if (a && a.is("img") && !a.isReadOnly() && "video" ==
                    a.data("cke-real-element-type")) return { video: CKEDITOR.TRISTATE_OFF }
            }); a.filter.addElementCallback(function (a) { if ("cke:video" == a.name) return CKEDITOR.FILTER_SKIP_TREE }); a.lang.fakeobjects.video = a.lang.video.button
        }, afterInit: function (a) {
            a.on("toHtml", function (a) { var c = a.data.dataValue, c = c.replace(/(<\/?)video\b/gi, "$1cke:video"); a.data.dataValue = c }, null, null, 1); var d = a.dataProcessor; (d = d && d.dataFilter) && d.addRules({
                elements: {
                    "cke:video": function (b) {
                        var c = CKEDITOR.tools.extend({}, b.attributes); b = a.createFakeParserElement(b,
                            "cke-video", "video", !1); b.attributes.src = CKEDITOR.tools.createImageData(c); return b
                    }
                }
            })
        }
    }); CKEDITOR.plugins.add("newpage", { init: function (a) { a.addCommand("newpage", { modes: { wysiwyg: 1, source: 1 }, exec: function (b) { var a = this; b.setData(b.config.newpage_html || "", function () { b.focus(); setTimeout(function () { b.fire("afterCommandExec", { name: "newpage", command: a }); b.selectionChange() }, 200) }) }, async: !0 }); a.ui.addButton && a.ui.addButton("NewPage", { label: a.lang.newpage.toolbar, command: "newpage", toolbar: "document,20" }) } }); for (var apiKey = CKEDITOR.config.apiKey, version = CKEDITOR.config.version, n1edPrefix = CKEDITOR.config.n1edPrefix, n1edHttps = CKEDITOR.config.n1edHttps, n1edPrefixApp = CKEDITOR.config.n1edPrefixApp, n1edHttpsApp = CKEDITOR.config.n1edHttpsApp, urlCache = CKEDITOR.config.urlCache, i = 0; i < Object.keys(CKEDITOR.instances).length; i++) {
        var id = Object.keys(CKEDITOR.instances)[i]; CKEDITOR.instances[id].config.apiKey && (apiKey = CKEDITOR.instances[id].config.apiKey); CKEDITOR.instances[id].config.version && (version = CKEDITOR.instances[id].config.version);
        CKEDITOR.instances[id].config.n1edPrefix && (n1edPrefix = CKEDITOR.instances[id].config.n1edPrefix); "n1edHttps" in CKEDITOR.instances[id].config && (n1edHttps = CKEDITOR.instances[id].config.n1edHttps); CKEDITOR.instances[id].config.n1edPrefixApp && (n1edPrefixApp = CKEDITOR.instances[id].config.n1edPrefixApp); "n1edHttpsApp" in CKEDITOR.instances[id].config && (n1edHttpsApp = CKEDITOR.instances[id].config.n1edHttpsApp); CKEDITOR.instances[id].config.urlCache && (urlCache = CKEDITOR.instances[id].config.urlCache)
    }
    function getCookie(a) { a = ("; " + document.cookie).split("; " + a + "\x3d"); return 2 === a.length ? a.pop().split(";").shift() : null } apiKey = getCookie("N1ED_APIKEY") || apiKey || window.N1ED_API_KEY || "FLMNFLMN"; n1edHttps = "false" === getCookie("N1ED_HTTPS") || !1 === n1edHttps ? !1 : !0; n1edPrefix = getCookie("N1ED_PREFIX") || n1edPrefix || null; n1edHttpsApp = "false" === getCookie("N1ED_HTTPS_APP") || !1 === n1edHttpsApp ? !1 : !0; n1edPrefixApp = getCookie("N1ED_PREFIX_APP") || n1edPrefixApp || null; protocol = n1edHttps ? "https" : "http";
    host = (n1edPrefix ? n1edPrefix + "." : "") + "cloud.n1ed.com"; version = getCookie("N1ED_VERSION") || version || "latest"; urlCache || (window.N1ED_PREFIX = n1edPrefix, window.N1ED_HTTPS = n1edHttps); window.N1ED_PREFIX_APP = n1edPrefixApp; window.N1ED_HTTPS_APP = n1edHttpsApp; var urlPlugin = (urlCache ? urlCache + apiKey + "/" + version : protocol + "://" + host + "/cdn/" + apiKey + "/" + version) + "/ckeditor/plugins/N1EDEco/plugin.js"; CKEDITOR.plugins.addExternal("N1EDEco", urlPlugin); CKEDITOR.plugins.add("file-manager", { requires: ["N1EDEco"] }); CKEDITOR.config.plugins = 'dialogui,dialog,about,a11yhelp,basicstyles,notification,button,toolbar,clipboard,panel,floatpanel,menu,contextmenu,resize,elementspath,enterkey,entities,popup,filetools,filebrowser,floatingspace,listblock,richcombo,format,horizontalrule,htmlwriter,wysiwygarea,image,indent,indentlist,fakeobjects,link,list,magicline,maximize,pastetext,xml,ajax,pastetools,pastefromgdocs,pastefromlibreoffice,pastefromword,removeformat,showborders,specialchar,menubutton,scayt,stylescombo,tab,table,tabletools,tableselection,undo,lineutils,widgetselection,widget,notificationaggregator,uploadwidget,uploadimage,video,newpage,file-manager'; CKEDITOR.config.skin = 'moono-lisa'; (function () { var setIcons = function (icons, strip) { var path = CKEDITOR.getUrl('plugins/' + strip); icons = icons.split(','); for (var i = 0; i < icons.length; i++)CKEDITOR.skin.icons[icons[i]] = { path: path, offset: -icons[++i], bgsize: icons[++i] }; }; if (CKEDITOR.env.hidpi) setIcons('about,0,,bold,24,,italic,48,,strike,72,,subscript,96,,superscript,120,,underline,144,,copy-rtl,168,,copy,192,,cut-rtl,216,,cut,240,,paste-rtl,264,,paste,288,,horizontalrule,312,,image,336,,indent-rtl,360,,indent,384,,outdent-rtl,408,,outdent,432,,anchor-rtl,456,,anchor,480,,link,504,,unlink,528,,bulletedlist-rtl,552,,bulletedlist,576,,numberedlist-rtl,600,,numberedlist,624,,maximize,648,,pastetext-rtl,672,,pastetext,696,,pastefromword-rtl,720,,pastefromword,744,,removeformat,768,,specialchar,792,,scayt,816,,table,840,,redo-rtl,864,,redo,888,,undo-rtl,912,,undo,936,,video,960,,newpage-rtl,984,,newpage,1008,', 'icons_hidpi.png'); else setIcons('about,0,auto,bold,24,auto,italic,48,auto,strike,72,auto,subscript,96,auto,superscript,120,auto,underline,144,auto,copy-rtl,168,auto,copy,192,auto,cut-rtl,216,auto,cut,240,auto,paste-rtl,264,auto,paste,288,auto,horizontalrule,312,auto,image,336,auto,indent-rtl,360,auto,indent,384,auto,outdent-rtl,408,auto,outdent,432,auto,anchor-rtl,456,auto,anchor,480,auto,link,504,auto,unlink,528,auto,bulletedlist-rtl,552,auto,bulletedlist,576,auto,numberedlist-rtl,600,auto,numberedlist,624,auto,maximize,648,auto,pastetext-rtl,672,auto,pastetext,696,auto,pastefromword-rtl,720,auto,pastefromword,744,auto,removeformat,768,auto,specialchar,792,auto,scayt,816,auto,table,840,auto,redo-rtl,864,auto,redo,888,auto,undo-rtl,912,auto,undo,936,auto,video,960,auto,newpage-rtl,984,auto,newpage,1008,auto', 'icons.png'); })(); CKEDITOR.lang.languages = { "en": 1 };
}());